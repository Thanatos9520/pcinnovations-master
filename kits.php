...
<html lang="en">
  ...
  <head>
    ...
    <?php include('conn.php');
     $query=mysqli_query($conn,"select * from product where categoryid = 7 order by product_name");  
    ...
    if(is_numeric($nro_pagina))
       ...
       $query=mysqli_query($conn,"select * from product where categoryid = 7 order by product_name limit $inicio,$reg_por_pagina");  
...
<div class="container">
    <div style="height: 50px;"></div>
     <h1 class="mt-4 mb-3">Kits/Combos
        <small></small>
      </h1>
          <div class="row">
        <div class="col-lg-12">
            <img src="img/Banner_Productos/partes.jpg" style="width:80%;">
        </div>
        </div>
    <div style="height: 50px;"></div>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Kits/Combos</li>
      </ol>
	...
	?>
      ...
      <?php
          if($nro_pagina>1)
              echo "<a href='kits.php?num=".($nro_pagina-1)."'> Anterior ></a> ";
              
       for ($i=1; $i<=$can_paginas; $i++){
           if ($i==$nro_pagina)
               echo $i." ";
           else 
               echo "<a href='kits.php?num=$i'>$i</a> ";
       } 
          
          if($nro_pagina<$can_paginas)
               echo "<a href='kits.php?num=".($nro_pagina+1)."'> Siguiente ></a> ";
