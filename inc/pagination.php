...
$query_num_product =  mysql_query("SELECT productid FROM product", $conexion);
$num_total_registros = mysql_num_rows($query_num_product);
...
//Si hay registros
 if ($num_total_registros > 0) {
    ...
    $query_services = mysql_query("SELECT productid, product_name FROM product WHERE categoryid=1 ORDER BY product_name DESC LIMIT $offset, $rowsPerPage", $conexion);
    while ($row_services = mysql_fetch_array($query_services)) {
                        //$service = new Service($row_services['service_id']);
		
		$descripcion_desformateada = strip_tags($row_services['product_name']);
        ...
        $p_desc = $texto.'...<br>';
        			...
        			echo '
						<div class="service_list" id="service'.$row_services['productid'].'" data="'.$row_services['productid'].'">
                         	
                            <div class="center_block">
                                <a title="'.$row_services['product_name'].'" class="product_img_link" href="#">
                                <img width="129" height="129" alt="'.$row_services['product_name'].'" src="upload/noimage.jpg"></a>
