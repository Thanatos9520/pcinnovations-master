...
<html lang="en">
  ...
  <head>
    ...
    <?php include('conn.php');
     $query=mysqli_query($conn,"select * from product where categoryid = 6 order by product_name");  
    ...
    if(is_numeric($nro_pagina))
       ...
       $query=mysqli_query($conn,"select * from product where categoryid = 6 order by product_name limit $inicio,$reg_por_pagina");  
...
<div class="row">
	...
	?>
      ...
      <?php
          if($nro_pagina>1)
              echo "<a href='gpu.php?num=".($nro_pagina-1)."'> Anterior ></a> ";
       ...
       for ($i=1; $i<=$can_paginas; $i++){
          ...
          if($nro_pagina<$can_paginas)
               echo "<a href='gpu.php?num=".($nro_pagina+1)."'> Siguiente ></a> ";
