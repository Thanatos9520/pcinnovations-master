...
<html lang="en">
  ...
  <head>
    ...
    if(is_numeric($nro_pagina))
       ...
       $query=mysqli_query($conn,"select * from product where categoryid = 6 order by product_price limit $inicio,$reg_por_pagina");  
...
<div class="container">
    <div style="height: 50px;"></div>
     <h1 class="mt-4 mb-3">Tarjeta de video
        <small></small>
      </h1>
          <div class="row">
        <div class="col-lg-12">
            <img src="img/Banner_Productos/1080ti.jpg" style="width:80%;">
        </div>
        </div>
    <div style="height: 50px;"></div>
      <ol class="breadcrumb">
...
<div class="row">
	...
	?>
      ...
      <?php
          if($nro_pagina>1)
              echo "<a style='background-color:#262424; border-radius:5px;' href='gpu.php?num=".($nro_pagina-1)."'> Anterior ></a> ";
              
       for ($i=1; $i<=$can_paginas; $i++){
           if ($i==$nro_pagina)
               echo $i." ";
           else 
               echo "<a style='color:black;' href='gpu.php?num=$i'>$i</a> ";
       } 
          
          if($nro_pagina<$can_paginas)
               echo "<a style='background-color:#262424; border-radius:5px;' href='gpu.php?num=".($nro_pagina+1)."'> Siguiente ></a> ";
