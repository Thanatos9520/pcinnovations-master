...
<html lang="en">
  ...
  <head>
    ...
    <?php include('conn.php');
     $query=mysqli_query($conn,"select * from product where categoryid = 19 order by product_name");  
    ...
    if(is_numeric($nro_pagina))
       ...
       $query=mysqli_query($conn,"select * from product where categoryid = 19 order by product_price limit $inicio,$reg_por_pagina");  
...
<div style="height: 50px;"></div>
      ...
      <div class="row">
        <div class="col-lg-3">
           <h1 style=" border-style: double; border-color:red; ">HEADSET</h1>
        </div>
        </div>
       <div class="row">
        <div class="col-lg-12">
            <img src="img/Banner_Productos/headset_banner.jpg" style="width: 100%;">
        </div>
        </div>
<div style="height: 50px;"></div>
  ...
  <div class="row">
	...
	?>
      ...
      <?php
          if($nro_pagina>1)
              echo "<a style='background-color:#262424; border-radius:5px;' href='headset.php?num=".($nro_pagina-1)."'> Anterior ></a> ";
              
       for ($i=1; $i<=$can_paginas; $i++){
           if ($i==$nro_pagina)
               echo $i." ";
           else 
               echo "<a style='color:black;' href='headset.php?num=$i'>$i</a> ";
       } 
          
          if($nro_pagina<$can_paginas)
               echo "<a style='background-color:#262424; border-radius:5px;' href='headset.php?num=".($nro_pagina+1)."'> Siguiente ></a> ";
